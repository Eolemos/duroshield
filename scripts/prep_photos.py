...
PHOTOS = {
    "aplicacao-planta-operacao": ("AplicacaoDuroshield1.jpg", 1280),
    "heliponto-concluido":       ("Heliponto-Depois-2-scaled-1.jpg", 2560),
    "bayer-cobertura":           ("Bayer-SJC-4.jpeg", 1024),
    "heineken-bacia-1":          ("HNK-Igrejinha-RS-71.jpg", 1280),
    "heineken-bacia-2":          ("HNK-Igrejinha-RS-52.jpg", 1280),
    "heineken-bacia-3":          ("HNK-Igrejinha-RS-79.jpg", 1280),
    "estacionamento-laje":       ("Daitan-scaled-1.jpg", 2560),
    "cobertura-fuvest":          ("Fuvest-1.jpg", 2048),
...
def emit(img: Image.Image, stem: str, width: int, suffix: str = "") -> int:
    ...
    if w > width:
        ...
        # Downscaling always softens; a light unsharp pass restores the edge the
        # resample cost. It cannot add detail the original never had.
        img = img.filter(ImageFilter.UnsharpMask(radius=1.1, percent=58, threshold=3))
    out = DST / f"{stem}{suffix}.webp"
    img.save(out, "WEBP", quality=93, method=6)
...
def main() -> int:
    ...
    for stem, (fname, width) in PHOTOS.items():
        ...
        # Narrower variants so each device pulls what its screen can actually show.
        mid = emit(img, stem, 1200, "@1200")
        thumb = emit(img, stem, 800, "@800")

        total_before += before
        total_after += full + mid + thumb
        print(f"  {w}x{h:<10} {before/1024:>6.0f}KB -> {full/1024:>5.0f}KB / "
              f"{mid/1024:>4.0f}KB / {thumb/1024:>4.0f}KB  {stem}.webp")
